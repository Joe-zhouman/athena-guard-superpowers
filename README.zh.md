...
插件只做一件事：注入一个 **SessionStart hook**，强制每个 session 开始时读取 `using-superpowers`。那个 skill 是引导程序——它教会代理如何找到和调用所有其他 skill。其他所有东西（20 个 skill——14 个核心 + 6 个补充、9 个代理、工作流）都由技能系统本身在引导程序运行后加载。
...
四个来自 [mattpocock/skills](https://github.com/mattpocock/skills) 的技能 + 两个 athena 原创，作为可选补充件捆绑在内。它们不参与 athena 主流程——放在这儿是为了你不用另外装：

| Skill | 做什么 | 什么时候用 |
|-------|--------|-----------|
| **diagnosing-bugs** | 6 阶段疑难 bug 诊断循环。先建反馈环，再假设、插桩、修复、回归测试 | bug 没有清晰复现路径、不知道哪个模块出了问题。cancer 处理有复现路径的 bug；diagnosing-bugs 处理其余所有 |
| **handoff** | 把当前对话压缩成交接文档给下一个 agent | 深入一个 session 后需要交给另一个 agent（或未来的自己） |
| **to-prd** | 把设计对话合成为 PRD 并发布到项目 issue tracker | 需要把内部设计输出为面向外部的 PRD。需要 `/setup-matt-pocock-skills` 配置 issue tracker |
| **prototype** | 构建一次性终端应用或 UI 变体来回答设计问题，答完就删 | 光靠 brainstorming 聊不清楚一个逻辑/状态模型，需要*跑起来*感受一下。分两条分支：logic（CLI）和 UI（多变体路由） |\n| **grill-me** | 不留情面的苏格拉底式盘问——一次一个问题，每个问题带推荐答案 | 压力测试任何 plan 或 design。就是那个启发 athena 的 brainstorming interview 风格的原版 Matt Pocock grill-me。不需要完整 brainstorming 流程时单独使用 |\n\n这些是**原封不动的 Matt Pocock 原版**——除了 grill-me，它是 athena 之前的原始版本。它们和 athena 的 skill 并行加载，随时可调用，不干扰主流程——你需要的时候它们就在那里。装了一个东西，全家桶都有了。
...
然后 ——**一行命令**（无需先 clone；脚本会问你 clone 到哪，从 GitHub clone、保留它可 git pull，然后安装）：

```powershell
# Windows（PowerShell 5.1+）—— 经 jsDelivr 拉脚本，从 GitHub clone
...
**给不熟终端的 Windows 用户：** 脚本会一步步引导——你在资源管理器里新建一个文件夹，从地址栏复制它的路径，粘贴到提示符下回车即可。路径不对它会告诉你哪里错了、让你重来，**绝不会在错误路径上往下走**。

脚本做的事：
1. 让你选（或新建）一个文件夹放仓库——仓库就留在那，可 git pull 更新。
2. 从 GitHub clone 到那个文件夹（已存在则 `git pull`）。
3. 跑仓库里的 `install.ps1`/`install.sh`，它会：
   - 把仓库 symlink 到 `~/.claude/skills/athena-superpowers/`——hooks 和 skills 通过 `@skills-dir` 自动加载
   - 把 9 个代理复制到 `~/.claude/agents/` 作为用户级全局代理——插件级代理会丢失 `permissionMode`/`mcpServers`（安全限制），而 athena 的代理需要这些能力

想**更新**就再跑一次同一行命令（git pull + 重装）。clone 目录**绝不删除**。
...
**补充件（4 个来自 Matt Pocock + 2 个 athena 原创）**
- **diagnosing-bugs**——6 阶段疑难 bug 诊断循环，没复现路径时用
- **to-prd**——把设计对话合成 PRD，发布到 issue tracker
- **prototype**——一次性代码，回答一个设计问题，答完就删
- **grill-me**——独立苏格拉底式盘问，压力测试任何 plan（slash only）
- **discuss-first**——先聊清楚再动手，别跳太快（slash only）
- **handoff**——压缩对话成交接文档，读完 `/handoff` 下一个 agent 就能接着干（slash only）
...
- `skills/`——20 个技能（14 个核心 + 4 个 Matt Pocock + 2 个 athena 原创），通过 `@skills-dir` 自动加载。引导入口：`skills/using-superpowers/SKILL.md`
...
| 资源 | 体积 | 等值 token |
|------|------|-----------|
| 20 个 skill SKILL.md（14 核心 + 6 补充） | 178 KB | ~44,500 tokens |

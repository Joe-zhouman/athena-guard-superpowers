...
插件只做一件事：注入一个 **SessionStart hook**，强制每个 session 开始时读取 `using-superpowers`。那个 skill 是引导程序——它教会代理如何找到和调用所有其他 skill。其他所有东西（19 个 skill——14 个核心 + 5 个补充、9 个代理、工作流）都由技能系统本身在引导程序运行后加载。
...
五个来自 [mattpocock/skills](https://github.com/mattpocock/skills) 的技能作为可选补充件捆绑在内。它们不参与 athena 主流程——放在这儿是为了你不用另外装：
...
- `skills/`——19 个技能（14 个核心 + 5 个来自 mattpocock/skills 的补充），通过 `@skills-dir` 自动加载。引导入口：`skills/using-superpowers/SKILL.md`
...
| 资源 | 体积 | 等值 token |
|------|------|-----------|
| 19 个 skill SKILL.md（14 核心 + 5 补充） | 177 KB | ~44,000 tokens |

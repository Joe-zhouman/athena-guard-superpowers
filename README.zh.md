...
插件只做一件事：注入一个 **SessionStart hook**，强制每个 session 开始时读取 `using-superpowers`。那个 skill 是引导程序——它教会代理如何找到和调用所有其他 skill。其他所有东西（18 个 skill——14 个核心 + 4 个补充、9 个代理、工作流）都由技能系统本身在引导程序运行后加载。
...
四个来自 [mattpocock/skills](https://github.com/mattpocock/skills) 的技能作为可选补充件捆绑在内。它们不参与 athena 主流程——放在这儿是为了你不用另外装：
...
这些是**原封不动的 Matt Pocock 原版**。它们和 athena 的 skill 并行加载，随时可调用，不干扰主流程——你需要的时候它们就在那里。装了一个东西，全家桶都有了。
...
- `skills/`——18 个技能（14 个核心 + 4 个来自 mattpocock/skills 的补充），通过 `@skills-dir` 自动加载。引导入口：`skills/using-superpowers/SKILL.md`
...
| 资源 | 体积 | 等值 token |
|------|------|-----------|
| 18 个 skill SKILL.md（14 核心 + 4 补充） | 176 KB | ~44,000 tokens |

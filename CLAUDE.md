...
- 10 zodiac-personified guardian subagents in `user-agents/` (capricorn / cancer / scorpio / taurus / libra / virgo / sagittarius / aries / aquarius / pisces), installed to `~/.claude/agents/` as user-level global agents. Named `user-agents/` (not `agents/`) so @skills-dir plugin auto-discovery won't load them as plugin agents.
- superpowers skill dispatch points rewired to call them by name (no more `general-purpose`)
- 5 core guardians (capricorn/scorpio/taurus/libra) auto-invoked by superpowers flow; cancer triggers on bug reports directly; 5 on-demand (virgo/sagittarius/aries/aquarius/pisces) dispatched by the main agent when needed

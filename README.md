...
**If you're cost-sensitive, think twice.** This is not a lightweight plugin. If your context window routinely fills up, or if you're on a tight token budget, this might not be for you. Upstream superpowers isn't lean either — if you want lightweight, grab [Matt Pocock's skills](https://github.com/mattpocock) and compose them yourself. In fact, this repo is designed to work *alongside* his skills — you can drop them in at any point in the workflow and they'll play nice together. The only coupling point is the file persistence layer (`docs/superpowers/`): athena agents read and write findings, specs, and reviews there. Matt's skills don't touch those paths, so there's no conflict. Everything else is mix-and-match.
